...
public class Acquirente extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("acquirenteLoggedIn")!=null && request.getParameter("accedi") != null) {
            ...
            try {
                if (OggettoFactory.getInstance().getOggettoById(idPar) != null) {
                    // creo l'oggetto selezionato in base all'id
                    Oggetto temp = OggettoFactory.getInstance().getOggettoById(Integer.parseInt(request.getParameter("productId")));
                    
                    // prendo dalla pagina precedente l'oggetto selezionato (dall'immagine del carrello) passato nella url
                    request.setAttribute("oggetto", OggettoFactory.getInstance().getOggettoById(Integer.parseInt(request.getParameter("productId"))));
...
"                                    </div>");
            ...
            float conto = temp.getConto().getSaldo();
